...
export default function Options({ info }) {
    ...
    const perticipantUser = info?.sender?.email !== loggedInUser?.email ? info.sender : info.receiver;
    ...
    const handelSubmit = (e) => {
        ...
        editConversation({
            ...
            data: {
                participants: `${loggedInUser.email}-${perticipantUser.email}`,
                users: [loggedInUser, perticipantUser],
                message,
    ...
    return (
        <form onSubmit={handelSubmit} className="flex items-center justify-between w-full p-3 border-t border-gray-300">
            <input
                type="text"
                placeholder="Message"
